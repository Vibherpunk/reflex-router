...
PROTECTED_DOMAINS = {"legal", "finance", "accounting", "compliance", "security"}
...
def classify_domain(messages: List[Dict[str, Any]]) -> Optional[str]:
    """
    Classifies a conversation request into a domain specialization (legal, medical, finance, math, accounting, compliance, security).
...
def classify_request(messages: List[Dict[str, Any]]) -> Tuple[int, str]:
    ...
    # 4. Check Tier 2 triggers (Deep Reasoning / Concurrency)
    ...
    if detected_domain in PROTECTED_DOMAINS:
        if re.search(r"(?i)\bblast[- ]radius\b", last_user_content):
            return 3, f"Domain hard floor (Tier 3 - high blast radius) enforced for '{detected_domain}'"
        return 2, f"Domain hard floor (Tier 2) enforced for '{detected_domain}'"

    # 6. Check Tier 1 triggers (General Implementation / Feature writing)
    for pattern in TIER1_PATTERNS:
        if re.search(pattern, last_user_content):
            return 1, f"Matched Tier 1 pattern: {pattern}"

    # 7. Length heuristic: substantive implementation threshold
    ...
    # 8. Default to Tier 0 (Fast / Cheap tool churn)
